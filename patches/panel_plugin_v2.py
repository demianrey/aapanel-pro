            ...
            if isinstance(_result, dict):
                ...
                if 'authorization' in _msg or 'not have' in _msg:
                    ...
                    if _sh_url:
                        ...
                        _pv0 = pluginInfo['versions'][0] if pluginInfo.get('versions') else {}
                        _req_v = _pv0.get('full_version', _pv0.get('m_version', ''))
                        _ver_fix = ''
                        if _req_v:
                            _ver_fix = (
                                '\n# Fix info.json version to match requested version\n'
                                'sleep 8 && /www/server/panel/pyenv/bin/python3 -c "'
                                'import json,os; p=\\"/www/server/panel/plugin/{n}/info.json\\"; '
                                'd=json.loads(open(p).read()) if os.path.exists(p) else {{}}; '
                                'd[\\"versions\\"]=\\"{v}\\"; open(p,\\"w\\").write(json.dumps(d))" &\n'
                            ).format(n=pluginInfo['name'], v=_req_v)
                        _wrap = ('#!/bin/bash\n'
                        ...
                        # Use the version the panel requested, not whatever the CDN has
                        _pv = pluginInfo['versions'][0] if pluginInfo.get('versions') else {}
                        _req_ver = _pv.get('full_version', _pv.get('m_version', ''))
                        if _req_ver:
                            _info['versions'] = _req_ver
